...
public class Update{
   ...
   public static void checkUpdate(){
      ...
      final double version = parseVersion(plugin.getDescription().getVersion());
      if(version > parseVersion(config.getString("version"))){
   ...
   /**
    * Parse the given input string for the version number.
    * 
    * @param in
    *           - Raw version.
    * @return Double value of the version.
    */
   private static double parseVersion(final String in){
      return Double.parseDouble(in.replace("-SNAPSHOT", ""));
   }
   ...
   private static void update(){
      ...
      plugin.saveConfig();
      plugin.getLogger().info("Upgrade to " + plugin.getDescription().getVersion() + " complete");
   }
}
