...
public class StorageHandler {
	...
	case MYSQL: {
	    mysql = new MySQL(plugin.getLogger(), "", config.host, config.port,
	...
	case SQLITE: {
		...
		sqlite = new SQLite(plugin.getLogger(), "", "storage", plugin
			.getDataFolder().getAbsolutePath());
		final Query query = sqlite
			.select("SELECT * FROM playerpoints;");
		final PreparedStatement statement = mysql
			.prepare(INSERT_PLAYER);
		if (query.getResult().next()) {
		    do {
			statement.setInt(1, query.getResult().getInt("points"));
			statement.setString(2,
				query.getResult().getString("playername"));
			statement.addBatch();
		...
		plugin.getLogger().warning(
			"SQLException on importSQL(" + source.toString() + ")");
		sql.printStackTrace();
	    }
	    break;
	}
